...
**Libft:** This repo’s **`libft/`** does **not** include **`get_next_line`**. Minishell reads lines with **`read`**, **`readline`**, and **`ft_read_stdin_line`** (`src/utils/msh_stdin_read_line.c`). Dropping GNL avoids an extra **static** stash in the library alongside the subject’s single allowed global (**`g_signum`**).
